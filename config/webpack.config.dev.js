...
const entryItems = paths.appIndexJs;

for(let i=0; i<entryItems.length; i++){
    if(entryItems[i].indexOf('registerServiceWorker') !== -1) continue;

    const key = path.basename(entryItems[i]).replace('.js', '');
    entry[key] = [require.resolve('react-dev-utils/webpackHotDevClient'), entryItems[i]];
}
